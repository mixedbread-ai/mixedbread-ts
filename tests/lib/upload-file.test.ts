...
function createMockClient(overrides?: {
  ...
  return { client, mockFetch, uploadsCreate, uploadsComplete, uploadsAbort };
}
...
describe('handleFileCreate', () => {
  ...
  test('small file delegates to standard POST /v1/files', async () => {
    ...
    const { client } = createMockClient({ postResult: expectedResult });
  ...
  test('large file uses multipart upload flow', async () => {
    ...
    const { client, uploadsCreate, uploadsComplete, mockFetch } = createMockClient({
      uploadsCreateResult: {
        id: 'upload-1',
        part_urls: [
    ...
    // Should upload 3 parts via client's fetch
  ...
  test('onPartUpload is not called for small files', async () => {
    const { client } = createMockClient({ postResult: { id: 'file-small' } });
