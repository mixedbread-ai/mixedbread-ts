...
export function createUploadCommand(): Command {
  const command = new Command("upload")
    ...
    .option("--parallel <n>", "Number of concurrent uploads", Number.parseInt)
    .option(
      "--unique",
  ...
  command.action(
    async (nameOrId: string, patterns: string[], options: UploadOptions) => {
      try {
        ...
        const parallel =
          mergedOptions.parallel || config.defaults?.upload?.parallel || 5;
        ...
        console.log(
          `Found ${uniqueFiles.length} files (${formatBytes(totalSize)})`
        );
...
async function uploadFiles(
  ...
  if (results.uploaded > 0) {
    console.log(
      chalk.green(`✓ ${results.uploaded} files uploaded successfully`)
    );
  }
...
async function uploadFromManifest(
  ...
  console.error(
    chalk.red("Error:"),
    "Manifest-based upload not yet implemented"
  );
  process.exit(1);
}
